<script setup>
    ...
    const submit = () => {
        requestData.get(route('admin.partner.index'), {
            preserveState: true,
            preserveScroll: true,
    ...
    const create = () => useForm().get(route('admin.partner.create'));

    const edit = (id) => useForm().get(route('admin.partner.edit', id));

    const show = (id) => useForm().get(route('admin.partner.show', id));
    ...
    function destroy(id) {
        $q.dialog({
            ...
            componentProps: {
                ...
                message: 'Tem certeza que deseja excluir essa parceiro?',
            },
        }).onOk(() => {
            useForm().delete(route('admin.partner.destroy', id), {
                onSuccess: () => {
                    $q.notify({
                        type: 'positive',
    ...
    function destroySelected() {
        $q.dialog({
            ...
            componentProps: {
                ...
                message: 'Tem certeza que deseja excluir parceiros selecionados?',
            },
      }).onOk(() => {
        useForm({ ids: selected.value.map(s => s.id) }).post(route('admin.partner.destroy-multiples'), {
            onSuccess: () => {
                selected.value = [];

                $q.notify({
                    type: 'positive',
                    message: 'Parceiros excluídos com sucesso',
                    position: 'top',
                })
            }
        })
    ...
    const clearFilters = () => useForm().get(route('admin.partner.index'))
    ...
    <AuthenticatedLayout>
        ...
        <div class="row q-pb-xl">
            <div class="column col-12 col-md-6 justify-center">
                <div class="adm-fs-28 adm-fw-700 adm-lh-32 text-grey-8"> Parceiros </div>
                ...
                >
                    ...
                    <q-breadcrumbs-el label="Parceiros" class="text-primary" />
                </q-breadcrumbs>
            </div>

             <div class="col-12 col-md-6 flex justify-end items-center">
                <q-btn
                    v-if="selected.length > 0 && canPartnerDestroy"
                    color="negative"
                    class="q-mr-md"
                    rounded
                    no-caps
                    ...
                    <template v-slot:body-cell-actions="props">
                        <q-td :props="props">
                            <div class="row items-center justify-center adm-fw-700 adm-fs-16">
                                <q-btn icon="more_vert" flat v-if="canPartnerEdit || canPartnerDestroy">
                                    <q-menu :offset="[65, 0]">
                                        <q-list>
                                            <q-item
                                                v-if="canPartnerEdit"
