<script setup>
    ...
    const submit = () => {
        requestData.get(route('admin.item.index'), {
            preserveState: true,
            preserveScroll: true,
    ...
    const create = () => useForm().get(route('admin.item.create'));

    const edit = (id) => useForm().get(route('admin.item.edit', id));

    const show = (id) => useForm().get(route('admin.item.show', id));
    ...
    function destroy(id) {
        $q.dialog({
            ...
            componentProps: {
                ...
                message: 'Tem certeza que deseja excluir essa brinde?',
            },
        }).onOk(() => {
            useForm().delete(route('admin.item.destroy', id), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
                    $q.notify({
    ...
    function destroySelected() {
        $q.dialog({
            ...
            componentProps: {
                ...
                message: 'Tem certeza que deseja excluir brindes selecionadas?',
            },
      }).onOk(() => {
        useForm({ ids: selected.value.map(s => s.id) }).post(route('admin.item.destroy-multiples'), {
            onSuccess: () => {
                selected.value = [];

                $q.notify({
                    type: 'positive',
                    message: 'Brindes excluídos com sucesso',
                    position: 'top',
                })
            }
        })
    ...
    const clearFilters = () => useForm().get(route('admin.item.index'))
    ...
    <AuthenticatedLayout>
        ...
        <div class="row q-pb-xl">
            <div class="column col-12 col-md-6 justify-center">
                <div class="adm-fs-28 adm-fw-700 adm-lh-32 text-grey-8"> Brindes </div>
                ...
                >
                    ...
                    <q-breadcrumbs-el label="Brindes" class="text-primary" />
                </q-breadcrumbs>
            </div>

             <div class="col-12 col-md-6 flex justify-end items-center">
                <q-btn
                    v-if="selected.length > 0 && canItemDestroy"
                    color="negative"
                    class="q-mr-md"
                    rounded
                    no-caps
            ...
            <q-card-section class="q-py-none">
                ...
                >
                    ...
                    <template v-slot:body-cell-actions="props">
                        <q-td :props="props">
                            <div class="row items-center justify-center adm-fw-700 adm-fs-16">
                                <q-btn icon="more_vert" flat v-if="canItemEdit || canItemDestroy">
                                    <q-menu :offset="[65, 0]">
                                        <q-list>
                                            <q-item
                                                v-if="canItemEdit"
