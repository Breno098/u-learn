<script setup>
    ...
    const submit = () => {
        requestData.get(route('admin.group.index'), {
            preserveState: true,
            preserveScroll: true,
    ...
    const create = () => useForm().get(route('admin.group.create'));

    const edit = (id) => useForm().get(route('admin.group.edit', id));

    const show = (id) => useForm().get(route('admin.group.show', id));
    ...
    function destroy(id) {
        $q.dialog({
            ...
            useForm().delete(route('admin.group.destroy', id), {
                onSuccess: () => {
                    $q.notify({
                        type: 'positive',
    ...
    function destroySelected() {
        ...
        useForm({ ids: selected.value.map(s => s.id) }).post(route('admin.group.destroy-multiples'), {
            onSuccess: () => {
                selected.value = [];
    ...
    const toStudents = () => useForm().get(route('admin.student.index'));

    const clearFilters = () => useForm().get(route('admin.group.index'))
    ...
    <AuthenticatedLayout>
        ...
        <div class="row q-pb-xl">
            <div class="column col-12 col-md-6 justify-center">
                <div class="adm-fs-28 adm-fw-700 adm-lh-32 text-grey-8"> Grupos </div>
                ...
                >
                    ...
                    <q-breadcrumbs-el label="Grupos" class="text-primary" />
                </q-breadcrumbs>
            </div>
