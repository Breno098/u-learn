<script setup>
    ...
    const props = defineProps({
        ...
        canCampaignStore: Boolean,
        canCampaignEdit: Boolean,
        canCampaignDestroy: Boolean,
    });
    ...
    const submit = () => {
        requestData.get(route('admin.campaign.index'), {
            preserveState: true,
            preserveScroll: true,
    ...
    const create = () => useForm().get(route('admin.campaign.create'));

    const edit = (id) => useForm().get(route('admin.campaign.edit', id));

    const show = (id) => useForm().get(route('admin.campaign.show', id));
    ...
    function destroy(id) {
        $q.dialog({
            ...
            componentProps: {
                ...
                message: 'Ao excluir a campanha, todas as campanhas vinculadas, serão desvinculadas. Deseja realmente excluir?',
            },
        }).onOk(() => {
            useForm().delete(route('admin.campaign.destroy', id), {
                onSuccess: () => {
                    $q.notify({
                        type: 'positive',
                        message: 'Campanha excluído com sucesso',
                        position: 'top',
                    })
                }
            })
    ...
    function destroySelected() {
        $q.dialog({
            ...
            componentProps: {
                ...
                message: 'Ao excluir as campanhas selecionadas, todas as campanhas vinculadas, serão desvinculadas. Deseja realmente excluir?',
            },
      }).onOk(() => {
        useForm({ ids: selected.value.map(s => s.id) }).post(route('admin.campaign.destroy-multiples'), {
            onSuccess: () => {
                selected.value = [];

                $q.notify({
                    type: 'positive',
                    message: 'Campanhas excluídas com sucesso',
                    position: 'top',
                })
            }
        })
    ...
    const clearFilters = () => useForm().get(route('admin.campaign.index'))
    ...
    <AuthenticatedLayout>
        ...
        <div class="row q-pb-xl">
            <div class="column col-12 col-md-6 justify-center">
                <div class="adm-fs-28 adm-fw-700 adm-lh-32 text-grey-8"> Campanhas </div>
                ...
                >
                    ...
                    <q-breadcrumbs-el label="Campanhas" class="text-primary" />
                </q-breadcrumbs>
            </div>

             <div class="col-12 col-md-6 flex justify-end items-center">
                <q-btn
                    v-if="selected.length > 0 && canCampaignDestroy"
                    color="negative"
                    class="q-mr-md"
                    rounded
                    no-caps
            ...
            <q-card-section class="q-py-none">
                ...
                >
                    ...
                    <template v-slot:body-cell-actions="props">
                        <q-td :props="props">
                            <div class="row items-center justify-center adm-fw-700 adm-fs-16">
                                <q-btn icon="more_vert" flat v-if="canCampaignEdit || canCampaignDestroy">
                                    <q-menu :offset="[65, 0]">
                                        <q-list>
                                            <q-item
                                                v-if="canCampaignEdit"
                                                clickable
