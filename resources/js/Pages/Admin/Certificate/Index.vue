<script setup>
    ...
    const props = defineProps({
        ...
        canCertificateStore: Boolean,
        canCertificateEdit: Boolean,
        canCertificateDestroy: Boolean,
    });
    ...
    const submit = () => {
        requestData.get(route('admin.certificate.index'), {
            preserveState: true,
            preserveScroll: true,
    ...
    const create = () => useForm().get(route('admin.certificate.create'));

    const edit = (id) => useForm().get(route('admin.certificate.edit', id));
    ...
    const destroy = (id) => {
        $q.dialog({
            ...
            useForm().delete(route('admin.certificate.destroy', id), {
                preserveState: true,
                preserveScroll: true,
                onSuccess: () => {
                    $q.dialog({
    ...
    const destroySelected = () => {
        $q.dialog({
            ...
            componentProps: {
                ...
                message: 'Ao excluir as certificados selecionadas, todos os cursos serão desvinculados. Deseja realmente excluir?',
                confirm: true,
                icon: { name: 'playlist_remove', color: 'red' }
            },
      }).onOk(() => {
        useForm({ ids: selected.value.map(s => s.id) }).post(route('admin.certificate.destroy-multiples'), {
            onSuccess: () => {
                selected.value = [];
                ...
                $q.dialog({
                    component: AdminDialog,
                    componentProps: {
                        message: 'Certificados excluídas com sucesso',
    ...
    const clearFilters = () => useForm().get(route('admin.certificate.index'))
</script>
        ...
        <q-card flat class="q-mb-lg">
            <q-card-section class="row items-center q-px-lg">
                <div class="flex col-12 col-md-6 justify-start items-center">
                    ...
                    <div class="adm-fs-28 text-blue-grey-10 q-ml-md"> Certificados </div>
                </div>
            ...
            <q-card-section class="q-pb-none q-pt-lg">
                    ...
                    <template v-slot:body-cell-actions="props">
                        <q-td :props="props">
                            <div class="row items-center justify-center adm-fw-700 adm-fs-16">
                                <q-btn icon="more_vert" flat v-if="canCertificateEdit || canCertificateDestroy">
                                    <q-menu :offset="[45, 0]">
                                        <q-list>
                                            <q-item
                                                v-if="canCertificateEdit"
                                                clickable
                ...
                <div class="row items-center text-grey">
                    ...
                    <q-pagination
                        ...
                        :max="certificates.meta.last_page"
                        @update:model-value="submit"
                        direction-links
                        boundary-links
                        color="indigo"
